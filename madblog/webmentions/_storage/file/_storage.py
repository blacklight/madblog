...
from .. import WebmentionsStorage
from ._watcher import ContentChange, ContentChangeType, FilesystemContentWatcher
...
class FileWebmentionsStorage(WebmentionsStorage):
    ...
    def __init__(
        ...
        **_,
        ...
        self._webmentions_watcher: FilesystemContentWatcher | None = None
    ...
    def retrieve_webmentions(
        ...
        for md_file in post_mentions_dir.glob("webmention-*.md"):
            try:
                ...
                if metadata.get("status") == WebmentionStatus.DELETED.value:
                    continue
    ...
    def start_watcher(self, webmentions_handler: WebmentionsHandler) -> None:
        with self._watcher_lock:
            if self._webmentions_watcher is not None:
                return

            self._webmentions_handler = webmentions_handler
            self._webmentions_watcher = FilesystemContentWatcher(
                root_dir=str(self.content_dir),
                on_change=self._on_filesystem_change,
            )
            self._webmentions_watcher.start()

    def stop_watcher(self) -> None:
        with self._watcher_lock:
            if self._webmentions_watcher is None:
                return

            self._webmentions_watcher.stop()
            self._webmentions_watcher = None

    def _source_url_from_content_path(self, content_path: str) -> str:
        ...
        base, _ = os.path.splitext(rel_str)
        base_link = self.base_url.rstrip("/")
        return f"{base_link}/article/{base.lstrip('/')}"
